...
private:
	...
	void rigid_aligment(const Eigen::Matrix<Scalar, num_thetas, 1>& theta_0, Eigen::Matrix<Scalar, num_thetas, 1> & theta, size_t permutation_index);	
	...
	void optimize_current_permutation(size_t permutation, size_t direction, const Eigen::Matrix<Scalar, num_thetas, 1> & theta_0, bool verbose, std::vector<float> & theta_std);
};
