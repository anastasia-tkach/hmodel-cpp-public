...
namespace energy {
	...
	PoseSpaceType pose_space_type; 

	void compute_pixels_to_axis_transform(cv::Mat & dataset_image) {
		pixels_length_x = dataset_image.cols;
		pixels_length_y = dataset_image.rows;
		axis_start_x = _Limits(0, 0);
		axis_start_y = _Limits(1, 0);
		float axis_end_x = _Limits(0, 1);
		float axis_end_y = _Limits(1, 1);
		ratio_x = (axis_end_x - axis_start_x) / pixels_length_x;
		ratio_y = (axis_end_y - axis_start_y) / pixels_length_y;
	}

	static void on_mouse_pca(int event, int pixels_x, int pixels_y, int f, void* ptr) {

		pixels_y = pixels_length_y - pixels_y;

		float axis_x = ratio_x * pixels_x + axis_start_x;
		float axis_y = ratio_y * pixels_y + axis_start_y;
		...
		if (pose_space_type == THUMB) {
			...
			for (int i = 0; i < theta_recovered.rows(); ++i) {
				theta_std[num_thetas_ignore + i] = theta_recovered[i];
			}
		}
		if (pose_space_type == FINGERS) {
			theta_recovered = _P * xk + _mu.segment(num_thetas_thumb, num_thetas_fingers);			
			for (int i = 0; i < theta_recovered.rows(); ++i) {
				theta_std[num_thetas_ignore + num_thetas_thumb + i] = theta_recovered[i];
			}
		}			
	...
	void PoseSpace::explore_pose_space(int type) {

		pose_space_type = (PoseSpaceType)type;
		char window_name[20] = "PCA Space";
		cv::Mat dataset_image;
		
		if (pose_space_type == THUMB) dataset_image = cv::imread(path_pca + "thumb/pca.png");
		if (pose_space_type == FINGERS) dataset_image = cv::imread(path_pca + "fingers/pca.png");				
		...
		compute_pixels_to_axis_transform(dataset_image);		
		cv::setMouseCallback(window_name, on_mouse_pca, &explore_mode);
	}

	void PoseSpace::init(Worker * worker) {
		this->worker = worker;
		this->path_pca = worker->data_path + "pose_space/";
