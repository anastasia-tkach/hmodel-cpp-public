...
namespace Eigen{
	inline QDebug operator<< (QDebug d, const Eigen::AlignedBox2i & box) { return d << "[" << box.min().x() << box.min().y() << box.min().z() << "]" << "[" << box.max().x() << box.max().y() << box.max().y() << "]"; }
	inline QDebug operator<< (QDebug d, const Eigen::AlignedBox3f & box) { return d << "[" << box.min().x() << box.min().y() << "]" << "[" << box.max().x() << box.max().y() << "]"; }
